...
- [ ] 音声・タイマー関連の数値を定数にする
- [ ] 進行バーやミリ秒換算など、その他の数値も定数にする
- [ ] 既存の `baseSpeedMultiplier` の命名を他の定数とそろえる

`claude_memo.html` の JS 部分に、意味の分からない数値がそのまま書かれている。
`2.0`（rate と playbackRate の上限、2 箇所）、`0.95`（rate の係数）、
`0.85`（pitch）、`6000` / `4.5` / `3000`（Web Speech の安全タイマー）、
`3000`（Online TTS の終了タイマーの余裕）、`180`（TTS へ渡す最大文字数）、
`50`（発話開始前の遅延）。`1.4` は `baseSpeedMultiplier` として定数に
なっているが、名前が他とそろっていない。

利用者と相談して、進行バーの `100` やミリ秒換算の `1000` のような
自明なものも含めて、JS 全体の数値を対象にすると決めた。
`slideData` の `id` と `duration` は原稿そのものなので触らない。
**挙動は変えない。**
