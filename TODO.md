...
**残っている項目: TODO-035、TODO-037、TODO-038。** これまでに 35 件を決着させた。
新しく足すときは「完了済み」の上に節を作る。**番号は `TODO-039` から。**
...
- [ ] 2 番目の箱に壁打ちを入れる
- [ ] ナレーションを直し、`duration` を測り直す

AI が TODO を起草してから利用者が承認するまでの間に、壁打ちして内容を
精査している。その工程が 2 番目の箱（「2. 起草・承認」）にもナレーションにも
無い。

- 箱: 見出しを「2. 起草・壁打ち・承認」、下の行を「AI起草 → 壁打ちで精査 → 承認」に
- ナレーション: 「AI が TODO を起草し、承認までの間に壁打ちしながら内容を
  精査します。承認後はスラッシュゴールで完走させます。」
- 本文が変わるので `tools/measure-duration.py 4` で `duration`（現在 19）を測り直す
