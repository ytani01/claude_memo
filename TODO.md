...
**残っている項目: TODO-035、TODO-039。** これまでに 37 件を決着させた。
新しく足すときは「完了済み」の上に節を作る。**番号は `TODO-040` から。**
...
- [ ] スライド 6 のグリッド下に注釈を 1 行足す
- [ ] ナレーション末尾にも一言足す
- [ ] duration を測り直す（tools/measure-duration.py）

スライド 6 の表は担当ごとにモデルが固定されているように読める。
実際はタスクの内容に応じて main が選んでいるので、その旨を注釈で補う。
