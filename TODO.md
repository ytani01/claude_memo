...
## TODO-020. ナレーション後の 2 秒待ちでも経過時間を進める

|      | main | 担当 |
|------|------|------|
| 見込み | Opus 5 / effort high | verifier + reviewer |

- [ ] 各スライドの尺に、読み終わり後の 2 秒待ちを含める
- [ ] 合計時間にも 2 秒 × 枚数を足す

ナレーションが終わってからスライドを切り替えるまでの 2 秒間、進行バーと
...
`duration + 2` にし、合計時間にも 2 秒 × 枚数を足す。2 秒は
`onSlideAudioFinished()` に直書きされているので、両方から参照できる形にする。
