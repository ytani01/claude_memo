...
**残っている項目: TODO-059、TODO-060、TODO-062、TODO-063、TODO-064、
TODO-065。**
これまでに 59 件を決着させた。
...
## TODO-062. スライドの見出しと本文の枠を `player.html` 側に寄せる
...
**見出しと、本文の外側の余白・行間までを `player.html` 側に寄せる。**
...
**「本文をどう組むか」の見本が無い。** TODO-062 で定型を寄せても、
...
- [ ] 新しい節を 1 つ作り、13 件を 1〜2 行ずつに圧縮して書く
- [ ] 4 と 10 は既にある記述の補強なので、元の箇条書きの側へ足す
- [ ] 13 件が漏れていないか、根拠の番号が合っているかを確かめさせる

TODO-061 の「トークン節約に効く順」のうち **1〜13 位を採る**。
14 位（候補 10. 入口や引数が増えるなら reviewer）と 15 位（候補 9. 実機）は
**採らない**。どちらもトークンが増える側で、今回の目的から外れる。

### 決めたこと

- **新しい節を 1 つ作る。** 「TODO.md でのタスク管理」の箇条書きは既に長く、
  そこへ 13 件を溶かすと読めなくなる
- **根拠は `yt_slide の TODO-028` の形で書く。** グローバルな
  `CLAUDE.md` なので、プロジェクト名を付けないとどの番号か分からない
  （既にある `tmr の TODO-016`、`ytsched の TODO-077` と同じ書き方）
