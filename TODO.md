...
- [ ] `slideData` の「現状の課題」を、16「OpenAI Codex との共存について」と
      17「まとめ」の間へ移す
- [ ] 枠の上の `SLIDE nn / 17` が配列順と `id` のどちらで出ているかを確かめ、
      ずれるなら `id` を振り直す
- [ ] 各スライドの本文・ナレーション・`duration` は変えない

今の並びでは、まだ何も説明していない 3 枚目で「トークン消費を減らしたい」
「Obsidian 連携を模索中」と言う。トークンの話は 6（/clear）と
14（複数アカウント）、ツール連携の話は 8（plugin と MCP）で出てくるので、
それを聞いたあとに「残っている課題」として置くほうが通る。
17（まとめ）が「自己流で手探りなので意見がほしい」で締まるため、
その直前に来る形になる。

2（主なコマンド一覧）はナレーションが「このあとの説明で登場する」という
予告なので 1 の直後のまま、12（記述例）は「タスクごとに担当エージェント」に
触れるので 11（マルチエージェント）の後のままとする。
