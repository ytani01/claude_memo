...
- [ ] Inworld の API キーを受け取る（利用者が取得する）
- [ ] `edge-tts` を入れる
- [ ] まとめスライドのナレーション（最長 152 字）を Inworld と edge-tts の
      両方で合成し、利用者が聴き比べる
- [ ] 差し替えるかどうかを決める

TODO-015 で「もっといい声にしたい」を一度見送っている。そのときの結論は
**事前生成した MP3 を同梱するしかなく、ナレーションを直すたびに作り直す
手間が重い**というもので、この前提は Inworld でも変わらない。静的ページに
API キーを置けないので、実行時に Inworld を呼ぶ道は無い。

**今回決めるのは「その手間に見合う音質差があるか」の 1 点だけ。**
判断は利用者が耳で行う。差し替えると決めたら、17 枚の生成と再生側の
差し替えは別項目にする。

Inworld TTS は日本語対応、従量 $25/100万文字から、試用枠 70 分程度。
ナレーション全 17 枚で 2,271 字なので、生成そのものの費用は問題にならない。
