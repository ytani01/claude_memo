...
- [ ] `CLAUDE.md` / `README.md` / `TODO.md` / `docs/User.md` / `docs/Developer.md` を推敲する
- [ ] 手順・コマンド・数値・構成が変わっていないことを確かめる

直すのは日本語だけ。直訳調、冗長な言い回し、用語のゆれを揃える。
**手順・コマンド・数値・見出しの意味は変えない。** `archives/` は対象外
（現行仕様ではないので触らない）。`CLAUDE.md` は指示文なので、
意味が変わらない範囲に限る。
