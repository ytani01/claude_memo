...
**残っている項目: TODO-050、TODO-051。** これまでに 49 件を決着させた。
...
- [ ] `tools/measure-duration.py` に `--write` を足す
- [ ] `--all --write` で 17 枚を測り、`slides/claude-memo.js` の
      `duration` を書き換える
- [ ] `docs/` と `CLAUDE.md` の測り方の記述に `--write` を足す

ナレーションを変えるたびに `duration` を測り直して手で書き写している
（TODO-018、TODO-030）。TODO-032 では、書き写し忘れで 1 枚ずれていた。
測る道具（`tools/measure-duration.py`）は TODO-030 で入れたので、
**測った値をそのままスライドデータへ入れるところだけ**が残っている。
...
- **要点だけを載せる**（各 8〜12 枚）。見出し単位で拾い、細かい注意書きは
  ドキュメント側へ誘導する。ドキュメントを全文なぞらない

**TODO-050 が決着してから着手する**（`tools/measure-duration.py` を
どちらも触るため）。
