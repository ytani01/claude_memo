...
- [ ] `claude_memo.html` の `slideData`（462〜1113 行）を
      `slides-claude-memo.js` へ移す。`deckConfig`（タイトル）も同じファイルに置く
- [ ] 残りを `player.html` にする。`?deck=<名前>` で読み込むデータを選ぶ
- [ ] `claude_memo.html` を `player.html?deck=claude-memo` へのリダイレクトにする
- [ ] 表示と再生が今までどおりか確かめる（verifier）

現状 1971 行の 1 ファイル。内訳は、1〜460 行が head・CSS・外枠の HTML、
462〜1113 行が `slideData`、1114〜1970 行が再生・TTS・字幕・進行バー。
エンジンがスライドから読むのは `title` / `duration` / `narration` / `render()`
の 4 つだけで、境界は既にできている。スライドを直すときに 860 行の
エンジンを読み込まずに済むので、トークンが減る。

共通側に残る固有の文字列は 3 箇所。8 行目の `<title>`、ヘッダーの
「私の Claude Code の使い方」、311 行目の `<span id="total-slides">17</span>`。
前の 2 つは `deckConfig` から流し込み、3 つ目は起動時に `slideData.length` で
上書きされているのでハードコードごと消す。

`render()` が返す Tailwind の生の HTML は、そのままデータ側に置く。
宣言的なレイアウトの語彙に置き換えるのは、17 枚それぞれレイアウトが違って
語彙が 17 種類できるので**やらない**。2 本目のスライドを作って共通の型が
見えてから考える。
