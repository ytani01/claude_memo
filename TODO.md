...
- [ ] `docs/User.md` の「公開」の節に、必要なファイルと前提を書く
- [ ] 挙げたファイルだけをコピーして動くことを確かめる

|      | main | 担当 |
|------|------|------|
| 見込み | Opus 5 / effort high | verifier |

`docs/User.md` の「公開」の節は「`public_html/` 下なので置くだけ」で
止まっていて、他のサーバーへ持っていくときに何を渡せばよいかが書いていない。
実際に要るのは `player.html`、`slides/_rules.js`、公開したいデッキの
`slides/<名前>.js` の 3 つだけで、`tools/`・`docs/`・`archives/` は要らない。
ネット接続（CDN）と HTTP 配信が前提になることも短く添える。

同じ趣旨は `docs/Developer.md` の「場所を選ばない」と `README.md` にあるが、
スライドを作るだけの人は `User.md` しか読まない。詳しくは Developer.md へ
リンクで送り、写しは置かない。
