...
- [ ] 既定のデッキを `claude-memo` から `readme` に変える（`player.html:466`）
- [ ] `README.md` と `docs/` からリンクする。既定が変わる分も直す
...
- **`?deck=` が無いときは `readme` を出す。** `claude-memo` は数あるデッキの
  1 つという扱いにして、既定から外す。`slides/readme.js` ができてから変える
  こと（先に変えると読めるデッキが無くなる）
